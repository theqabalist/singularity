...
module.exports = (function (_) {
    ...
    return {
        data: data,
        builders: {
            lift: _.curry(function (type, f, t) { return t[type].from(_.curry(f)); }),
            map: function (rebuild, ret) {
                return function () {
                    var args = _.toList(arguments),
                        t = _.last(args),
                        f = _.last(_.initial(args)),
                        cargs = _.initial(_.initial(args));
                    return t[rebuild].from.apply(null, cargs).flatMap(function (t1) {
                        return t[ret].from(f(t1));
                    });
                };
            },
            ap: function (rebuild, ret) {
                return function () {
                    var args = _.toList(arguments),
                        t = _.last(args),
                        m = _.last(_.initial(args)),
                        cargs = _.initial(_.initial(args));
                    return t[rebuild].from.apply(null, cargs).flatMap(function (f) {
                        return m.flatMap(function (t1) {
                            return t[ret].from(f(t1));
                        });
                    });
                };
