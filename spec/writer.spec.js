...
describe("Writer Type", function () {
    ...
    describe(".mreturn", function () {
        it("should take a value and return a trivial writer", function () {
            var w = Writer.mreturn(Arr.mzero(), 0),
                log = w.val(getLog);
            expect(log).toEqual([]);
        });
    });
    ...
    describe("fmap", function () {
        it("should implement functor mapping", function () {
            /*jslint unparam: true*/
            var w = Writer.mreturn(Arr.mzero(), 0).map(function (x) { return x + 1; }),
    ...
    describe("#output", function () {
        it("should provide a convenience method for the output", function () {
            var log = Writer.typed(Arr).mreturn(0).output();
    ...
    describe("#data", function () {
        it("should provide a convenience method for the data", function () {
            var data = Writer.typed(Arr).mreturn(0).data();
            expect(data).toBe(0);
        });
    });
});